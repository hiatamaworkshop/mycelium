...
### Phase 0: mycelium-core 抽出

tick エンジン・species・metabolism を共通パッケージとして切り出し。

- [ ] `src/core/` から feeder.ts, pushback 関連を除いたコアを分離
- [ ] `resolveSpecies()`, `createNode()`, `nodeToPayload()`, `payloadToNode()` — そのまま
- [ ] metabolism.json, species-mapping.json, species.json — そのまま
- [ ] tick, receptor, digestor, observatory — そのまま
- [ ] npm パッケージ or git submodule として公開

### Phase 1: 汎用ローダー最小構成（単一インスタンス）

Source Qdrant → Mycelium Qdrant のパイプラインを1本通す。

- [ ] `SourcePoint` インターフェース定義
- [ ] `scrollSourcePoints()` — 汎用 Qdrant scroll（feeder.ts の `scrollEngramNodes()` を汎用化）
- [ ] `parseTags()` — キーワードベースの Tag Parser（過渡期補助）
- [ ] Batch Inject: `SourcePoint → parseTags → resolveSpecies → createNode → upsertPoints`
- [ ] `node.sourceId`, `node.chunkSeqNo` フィールド追加
- [ ] 固定 tick 数で打ち切り → 生存ノード scroll → `SurvivorReport` 生成
- [ ] harvest: sourceId 集計、survivalRate 計算、共鳴マップ生成

### Phase 2: カスケード並列化

Dispatcher + 複数インスタンスの協調。

- [ ] Dispatcher: sourceId 単位で記事をインスタンスに振り分け
- [ ] カスケード起動: A 開始 → ~20% 進行で pause → B inject → A resume → ...
- [ ] 各インスタンス独立 tick カウンタ、順次完了・順次 harvest
- [ ] inject 時 pause 制御（Qdrant I/O 衝突回避）
