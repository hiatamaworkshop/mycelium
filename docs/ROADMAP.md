...
> Status: Phase 1 ✅, Phase 1b ✅, Phase 2 ✅, Phase 2b ✅ (MCP server), Phase F0–F3 ✅（燃料ループ完結）, Phase 3 ✅（engram キャッシュ統合、engram 側実装）, Phase 4a 実装済み・効果未検出（保留） — 次手: Phase 4b or 4c
...
（`main.ts`, 1-3, デフォルト1）として実装済み。コア変更ゼロ（`createNode` は
既に `inheritedDelta` 引数を持っていた）。`npx tsc --noEmit` クリーン。

**検証結果 (2026-07-18) — 効果未検出**:
実データ（source_patent, 9ファイル）で round 1 の生存者集合を固定し、
round 2 を DeltaPool あり/なしで各20回試行して平均生存率を比較。

```
平均: WITH pool 15.4% vs WITHOUT pool 15.0%（差 0.4pt）
全9ファイルで差は ±1.1pt 以内 — ほぼ全ファイルで標準偏差より小さい
```

DeltaPool 引継ぎは生存率にほぼ影響しなかった。単発試行で観測した
「round2で67-100%生存」という見かけの効果は、DeltaPool ではなく
round1生存者集合の構成そのもの（凍結すれば低分散に収束）による
ものだった可能性が高い。推測される原因: `learnedDelta` は
`personality × (1 + learnedDelta)` の乗算補正（`deltaClamp=0.5` で
±50%程度）に過ぎず、1周分の学習量では種族デフォルトの personality
を覆すほど育っていない。

**結論**: 4a は保留。単純な1周引継ぎでは効果が測定できないことが
確認できた。再挑戦するなら (a) 3周以上重ねてδを蓄積させても無効か、
(b) δのスケールを増幅して理論上の効果が出る範囲か、のいずれかの
追加検証が前提。4b/4c には依存しないため、いつでも再開可能。
...
Phase 4a (実験)       ✅ 実装済み・効果未検出（保留） — DeltaPool A/B で生存率への
                      有意差なし（差0.4pt、全ファイル±1.1pt以内）。詳細は上記セクション参照
Phase 4b/4c (実験)    ← 次手。Phase 1/1b の制御基盤があると効率的
...
別解。4a は実装・A/B 検証済みだが生存率への効果は測定できなかった（上記参照）。
燃料ループ（F1-F3）側は別の実装・別の検証結果を持つため、4a の結果はそちらの
評価には影響しない。
