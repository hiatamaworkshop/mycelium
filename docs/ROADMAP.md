...
> Status: Phase 1 ✅, Phase 1b ✅, Phase 2 ✅, Phase 2b ✅ (MCP server), Phase F0 ✅ — 次手: Phase F1（燃料ループ）
...
**F1: NutritionResolver + 呼び出し側書き戻し（次手）**
- `myceliumMetrics` を読み、weight と合成して初期 w/h/d を解決する
  `NutritionResolver` を inject() 前段に追加
- レポートに書き戻し用メトリクス差分（pointId ごとの survived / lastClass /
  consensusRate）を含める
- receptor sink 側: レポートを受け取り source Qdrant の `myceliumMetrics` に適用
  （mycelium リポジトリ外、subsystem 統合の一部）
...
Phase F1 (燃料解決)   ← F0 に依存。次手。書き戻し適用は receptor sink 統合と並走
Phase F2 (利用シグナル) ← F1 + receptor/sink の hits/reads push に依存
