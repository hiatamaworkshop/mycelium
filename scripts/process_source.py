...
TAG_RULES: list[tuple[str, list[str]]] = [
    ...
    # ---- Academic: sentinel (rigid / formal / constraining) ----
    ("methodology",[r"\bour method\b", r"\bour approach\b", r"\btechnique\b",
                    r"\balgorithm\b", r"\bprocedure\b", r"\bframework\b",
                    r"\bformulation\b", r"\bscheme\b"]),
    ("definition", [r"\bis defined (?:as|by)\b", r"\bdenoted by\b", r"\blet \w+ be\b",
                    r"\bwe define\b", r"\bdefinition\b"]),
    ("theorem",    [r"\btheorem\b", r"\blemma\b", r"\bcorollary\b", r"\bproof\b",
                    r"\bproposition\b", r"\bq\.?\s*e\.?\s*d\b"]),
    ("constraint", [r"\bnecessary condition\b", r"\bsufficient condition\b",
                    r"\bif and only if\b", r"\bimplies that\b", r"\bit follows that\b",
                    r"\bprovided that\b", r"\bgiven that\b",
                    r"\brequires that\b", r"\bmust satisfy\b"]),
    ("bound",      [r"\bupper bound\b", r"\blower bound\b", r"\bbounded by\b",
                    r"\bin the limit\b", r"\basymptotic(?:ally)?\b",
                    r"\bconvergence\b", r"\bconverges?\b"]),
    ("caveat",     [r"\bhowever\b", r"\bin contrast\b", r"\bwhereas\b",
                    r"\bnevertheless\b", r"\bon the other hand\b",
                    r"\blimitation\b", r"\bcaveat\b", r"\bassumption\b",
                    r"\bit should be noted\b", r"\bwe note that\b"]),
    ("comparison", [r"\bcompared (?:to|with)\b", r"\boutperforms?\b",
                    r"\bbaseline\b", r"\bstate[- ]of[- ]the[- ]art\b",
                    r"\bprior work\b", r"\bprevious(?:ly)?\b",
                    r"\bin agreement with\b", r"\bconsistent with\b"]),
    # ---- Dev/Ops: sentinel (quality / observability) ----
    ("performance",[r"\bperformance\b", r"\boptimi[sz](?:e|ation)\b", r"\bbenchmark\b",
                    r"\blatency\b", r"\bthroughput\b", r"\bprofil(?:e|ing)\b"]),
    ("debug",      [r"\bdebug(?:ging)?\b", r"\blogging\b", r"\btrace\b",
                    r"\bstack trace\b", r"\breproduce\b"]),
    ("monitoring",  [r"\bmonitor(?:ing)?\b", r"\bmetric\b", r"\balert(?:ing)?\b",
                    r"\bobservability\b", r"\bdashboard\b"]),
    ("obsolete",   [r"\bobsolete\b", r"\bdeprecated\b", r"\bno longer (?:used|supported)\b",
                    r"\bend[- ]of[- ]life\b"]),
    # ---- Dev/Ops: herald (change / discovery) ----
    ("refactor",   [r"\brefactor(?:ing)?\b", r"\brestructur(?:e|ing)\b",
                    r"\bcleanup\b", r"\breorgani[sz](?:e|ation)\b"]),
    ("experiment", [r"\bexperiment(?:al|s)?\b", r"\bmeasurement\b",
                    r"\bour data\b", r"\bdataset\b"]),
    # ---- Dev/Ops: summarizer (overview / dependency) ----
    ("dependency", [r"\bdependenc(?:y|ies)\b", r"\blibrary\b", r"\bpackage\b",
                    r"\bupgrade\b", r"\bdowngrade\b"]),
    # ---- Dev/Ops: spore (tentative / throwaway) ----
    ("temporary",  [r"\btemporary\b", r"\bworkaround\b", r"\bhack\b",
                    r"\bquick fix\b"]),
    # ---- Academic: herald (discovering / announcing) ----
    ...
    # ---- Academic: summarizer (additive / overview) ----
